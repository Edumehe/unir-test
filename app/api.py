...
@api_application.route("/calc/squarert/<op_1>", methods=["GET"])
def squarert(op_1):
    try:
        num_1 = util.convert_to_number(op_1)
        return ("{}".format(CALCULATOR.squarert(num_1)), http.client.OK, HEADERS)
    except TypeError as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)
    
@api_application.route("/calc/lg10/<op_1>", methods=["GET"])
def lg10(op_1):
    try:
        num_1= util.convert_to_number(op_1)
        return ("{}".format(CALCULATOR.lg10(num_1)), http.client.OK, HEADERS)
    except TypeError as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)